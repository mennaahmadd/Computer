...
public class Computer {
    private Parser parser; // ama nygy nhot el word f memory hwa da el hay7wlha men assembly l binary 
    final int instStart = 0; //instruction start 
    final int instEnd = 1023;
    final int dataStart = 1024; 
    final int dataEnd = 2047;
    final int memSize = 2048;
   // int dataSize;
    int instructionSize; // size instruction alb2rha mn el file 
    Instruction stages[]; // 5 stages 
    int memory[]; //array of integers 
    int registers[];  //array of integers 
    int pc; 
    int cycles; 
    ...
    public Computer() {
        ...
        pc = 0;
       // dataSize = 0;
        instructionSize = 0; //size el instruction el ha3mlha read men el file
    }

    public void fetch() {
        if (pc >= instructionSize)
            return; // done with all instructions no instructions left 
        int inst = memory[pc];
        stages[0] = new Instruction(); // new instruction w ahotha f fetching stage
        stages[0].fetch(inst); // btakhod binary instruction mn memory 
        pc++;
    }
    ...
    public void writeBack() {
        ...
        Instruction curInstruction = stages[4]; // eh daaaa???
        ...
        switch (opcode) {
            ...
            case 10:
                ...
                break; // curinstruction.r1 address of r1 
        ...
        while (pc < instructionSize || !allNull()) {
            if (cycles % 2 == 1) {
               ...
               // stages[0] = null;
                ...
                execute();
                fetch();
            // sol for data hazards 
            // inst t write back abl tanya ma teexecute 
            } else {
                memoryAccess();
                decode();
            }
    ...
    private boolean checkJump() {
        ...
        // ba3rf al adress el hjump leeh f el execute stage 
        if (stages[3] == null )
            return false;
        ...
        if(stages[3].jumpFlag){
            ...
            pc = stages[3].aluResult; // el hasbtha f execute 
            stages[0] = null;
            stages[1]= null;
            stages[2] = null;
            return true;
    ...
    private void shiftStages(int cycles) {
        // prepare stages for the next clock cycle
         if(cycles%2 == 1){
            ...
            for(int i=4;i>0;i--)
                stages[i] = stages[i-1];
            stages[0] = null; //3ashan yb2a fee makan ll instruction
        }else{
