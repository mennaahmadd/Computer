...
public class Instruction {
    int binInstruction;
    String stringInstruction; //assembly instruction
...
//      System.out.println(instruction+" "+inst);
    ...
    public void execute(int pc, int[] registerFile){
        ...
        switch (opcode){
            ...
            case 4 : if( valueR1 == valueR2 ){
                aluResult= pc+imm - 1;  // 3ayza arg3lhaaa
                jumpFlag = true; //3ashan n3rf hay7sl jump wla laaa
            }
    ...
    public String convertBinaryToAssembly(int instruction) {
        ...
        // R-type
        int opcode = Integer.parseInt(inst.substring(0, 4), 2); //base 2 
        ...
        switch (opcode){
            ...
            // jump to  PC+1+IMM no registers needed
            // PC is already pointing to next instruction,add immediate to it
    ...
    public static String numberTobinaryString(int n) {
        ...
        //1101
        //0001
        return sb.reverse().toString(); //reverse ba3d ma et3wg 
    }

    public static int parseSignedInteger(String binString){
        // if it's positive, just parse
        if(binString.charAt(0)=='0') // awl bin 3l shemal sign bit
            return Integer.parseInt(binString, 2);  //hayrg3 nafs el rakm bs integer 

        // it's a negative number, flip bits and add one
        StringBuilder sb = new StringBuilder();
        for(int i=0;i<binString.length();i++)
            sb.append(binString.charAt(i)=='0'?'1':'0');
        int val = Integer.parseInt(sb.toString(), 2) + 1; // tostring 3ashan da string builder 
        return -val;
    }
